...
async function sendEmail(to, subject, text, html) {
  try {
    ...
    console.error('--- EMAIL DELIVERY FAILED (FALLBACK LOGGING) ---');
    console.error(`To: ${to}`);
    console.error(`Subject: ${subject}`);
    console.error(`Content: ${text || 'HTML Content'}`);
    console.error('-----------------------------------------------');

    return { messageId: 'mock-id-' + Date.now(), response: 'Logged to console' };
  }
}
